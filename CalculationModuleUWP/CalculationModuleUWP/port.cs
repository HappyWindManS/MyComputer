...
namespace CalculationModuleUWP
{
    ...
    public class port
    {
        private Hashtable titleHash = new Hashtable();
        public Hashtable ProblemGeneration(string QuestionNumber,string grade)
        {
            switch(grade)
            {
                case "一年级":
                    for(int i=0;i<int.Parse(QuestionNumber);i++)
                    {
                        ...
                        titleHash.Add(title, CalculationLogic.ReversePolishType(title));
                    }
                    break;
                case "二年级":
                    for (int i = 0; i < int.Parse(QuestionNumber); i++)
